...
void Request::parse_request(std::string request) {
  ...
  std::erase_if(request_tokens, [](auto &&str) { return str.length() == 0; });
  ...
  int count = std::count(request_tokens.begin(), request_tokens.end(),
                         "Content-Type: application/json") +
              std::count(request_tokens.begin(), request_tokens.end(),
                         "content-type: application/json");

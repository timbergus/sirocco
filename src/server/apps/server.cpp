...
Server::Server(std::map<std::string, std::any> options) {
  if (auto public_path{options.find("public_path")};
      public_path != std::end(options)) {
...
void Server::handle_response(std::string verb, std::string request_path,
                             std::function<void(Comms)> callback) {
  handlers[verb + "_" + request_path] = callback;
}

void Server::get(std::string request_path,
                 std::function<void(Comms)> callback) {
  log("GET " + request_path);
  handle_response("GET", request_path, callback);
}

void Server::post(std::string request_path,
                  std::function<void(Comms)> callback) {
  log("POST " + request_path);
  handle_response("POST", request_path, callback);
}

void Server::put(std::string request_path,
                 std::function<void(Comms)> callback) {
  log("PUT " + request_path);
  handle_response("PUT", request_path, callback);
}

void Server::patch(std::string request_path,
                   std::function<void(Comms)> callback) {
  log("PATCH " + request_path);
  handle_response("PATCH", request_path, callback);
}

void Server::del(std::string request_path,
                 std::function<void(Comms)> callback) {

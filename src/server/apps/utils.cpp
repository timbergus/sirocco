...
void split(std::string_view sample, std::string_view delimeter,
           std::vector<std::string> &tokens) {
  std::string::size_type beg = 0;

  for (auto end = 0; (end = sample.find(delimeter, end)) !=
...
std::string read_file(std::string_view file_name) {
  ...
  if (file.is_open()) {
    ...
    std::cerr << "Could not open file " << file_name << " :" << strerror(errno)
              << std::endl;
    return static_cast<std::string>("");
  }
}

void read_env_file(std::string_view env_file,
                   std::map<std::string, std::string> &env) {
  std::ifstream file{env_file.data()};

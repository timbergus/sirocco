...
static int get_users_callback([[maybe_unused]] void *NotUsed, int argc,
...
static int post_users_callback([[maybe_unused]] void *NotUsed,
...
const auto post_users = [](Comms comms) {
  try {
    ...
    comms.send_contents(
        fmt::format("<h1>Unexpected server error</h1><br><p>{}</p>", e.what()),
        "html", 500);
  }
};
